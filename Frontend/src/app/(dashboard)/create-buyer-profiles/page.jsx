...
import { createOrUpdateBuyerProfile, getBuyerProfile, updateBuyerProfile } from "@/actions/buyerProfile";
import { toast } from "sonner";

export default function CreateBuyerProfilePage() {
  ...
  // For create page, we're always creating (not updating)
  const isUpdate = false;

  // For create page, we don't load existing data - start with empty form
  useEffect(() => {
  ...
  const handleTagDataAdd = () => {
    ...
    console.log("🏷️ Tag name trimmed:", `"${newTagData.tag_name.trim()}"`);
    console.log("🏷️ Tag name length:", newTagData.tag_name.length);
    console.log("🏷️ Tag name truthy:", !!newTagData.tag_name.trim());
    
    if (newTagData.tag_name.trim()) {
      const newTag = { ...newTagData, tag_name: newTagData.tag_name.trim() };
      console.log("🏷️ New tag object:", newTag);
      
      setFormData(prev => {
        const updatedTags = [...prev.tags, newTag];
        console.log("🏷️ Updated tags array:", updatedTags);
        console.log("🏷️ Previous tags:", prev.tags);
        return {
          ...prev,
          tags: updatedTags
        };
      ...
      console.log("🏷️ Tag name is empty, not adding tag");
      console.log("🏷️ Please enter a tag name first!");
    }
  };

  const handleTagDataRemove = (indexToRemove) => {
    setFormData(prev => ({
      ...prev,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log("🔍 Operation type: CREATE");
      
      const result = await createOrUpdateBuyerProfile({
      ...
      console.log("Buyer profile creation result:", result);

      if (result.status === "success") {
        toast.success("Buyer profile created successfully!");
        router.push("/buyer-profile");
      } else {
        toast.error(result.message || "Failed to save buyer profile");
      }
    } catch (error) {
        ...
        <Container>
          <div className="flex items-center justify-center py-12">
            <div className="text-center">
              ...
              <p className="text-gray-600">Loading create buyer profile form...</p>
            </div>
          </div>
        </Container>
      </main>
      ...
      <Container>
        <div className="mb-6">
          ...
          <p className="text-gray-600 mt-2">
            Set up your buyer preferences and interests
          </p>
        </div>
