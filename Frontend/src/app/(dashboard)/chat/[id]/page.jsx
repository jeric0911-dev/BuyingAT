...
import { getMessagesById, sentMessage, getAllConversation, getPusherConfig } from "@/actions/others";
...
export default function ChatPage() {
  ...
  // Determine the other participant early
  const otherParticipant = conversation && currentUser ? 
  ...
  useEffect(() => {
    if (conversation && currentUser && otherParticipant) {
      checkOnlineStatus();
      // Check status every 30 seconds
      const interval = setInterval(checkOnlineStatus, 30000);
      return () => clearInterval(interval);
    }
  }, [conversation, currentUser, otherParticipant]);
    ...
    const loadChatData = async () => {
      try {
        ...
        if (conversationsResult.status === 'success') {
          const foundConversation = conversationsResult.data.find(conv => conv.id == conversationId);
          console.log('Found conversation:', foundConversation);
          setConversation(foundConversation);
          // fetch member status for other participant
          const other = foundConversation ? (foundConversation.sender_id === user.id ? foundConversation.receiver : foundConversation.sender) : null;
          if (other?.id) {
            try {
              const status = await getUserPackageStatus(other.id);
              setIsMember(!!status?.data?.active);
            } catch (_) {}
          }
          
          // If conversation not found, try to fetch it directly
          if (!foundConversation) {
            console.log('Conversation not found in list, trying direct fetch...');
            // You could add a direct API call here if needed
          }
        }
        
        // Get messages
        const messagesResult = await getMessagesById(conversationId);
        if (messagesResult.status === 'success') {
          setMessages(applyDealRemovals(messagesResult.data.reverse())); // Reverse to show oldest first
  ...
  useEffect(() => {
    if (pusherConfig && conversationId) {
      console.log('🔌 Initializing Pusher WebSocket connection for conversation:', conversationId);
    ...
    return (
      <div className="flex items-center justify-center h-screen">
        <div className="text-center">
          ...
          <button 
            onClick={() => router.push('/buyer-profiles')}
  ...
  return (
    <div className="h-screen flex flex-col lg:flex-row overflow-hidden min-h-0">
             ...
             <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">
                  {conversation?.conversation_type === 'buyer_profile' ? 'Buyer Profile Chat' : 'Product Chat'}
                </p>
              </div>
            </div>
          </div>
      ...
      {otherParticipant?.id && (
        <DealBuilder
          sellerId={conversation?.conversation_type === 'product' ? otherParticipant.id : currentUser?.id}
          buyerId={conversation?.conversation_type === 'buyer_profile' ? otherParticipant.id : null}
          ...
          fetchSellerListings={async (sellerId, search) => {
            ...
            return list
              ...
              .map((c) => {
                // Use seller_inventory_id if available (explicitly added by backend)
                // Otherwise fall back to seller_inventory.id or card_id
                // This ensures we always use the correct seller_inventory.id
                const sellerInventoryId = c.seller_inventory_id || c.seller_inventory?.id || c.card_id;
                
                // Log for debugging if IDs don't match
                if (c.seller_inventory_id && c.card_id && c.seller_inventory_id !== c.card_id) {
                  console.warn('ID mismatch detected:', {
                    card_id: c.card_id,
                    seller_inventory_id: c.seller_inventory_id,
                    seller_inventory_object_id: c.seller_inventory?.id,
                    card_title: c.card_title
                  });
                }
                
                return {
                  // Use seller_inventory_id - this is the correct seller_inventory.id
